...
class CmcQuoteServiceTest {

    CmcQuoteService cmcQuoteService;

    JSONObject jsonObjectUnderTest;

    @BeforeEach
    void init() {
        cmcQuoteService = new CmcQuoteService();
    ...
    void extractJsonArrayTest() {
        JSONArray jsonArray = cmcQuoteService.extractJsonArray(new JSONObject("{\"data\":[{\"id\":1}]}"), "data");
    ...
    void extractJsonObjectTest() {
        JSONObject jsonObject = cmcQuoteService.extractJsonObject(new JSONObject("{\"data\":{\"id\":1}}"), "data");

        assertNotNull(jsonObject);
    }

    @Test
    void extractUniqueObjectKeysTest() {
        Map<String, Object> keyList = new HashMap<>();
        keyList = cmcQuoteService.extractKeyPairs(jsonObjectUnderTest, keyList, true);

        assertTrue(keyList.size() > 0);
    }

    @Test
    void flatMapJsonTest() {
        // setup
        Map<String, Object> keyList = new HashMap<>();
        keyList = cmcQuoteService.extractKeyPairs(jsonObjectUnderTest, keyList, true);

        // under test
        Map<String, String> mapEntry = cmcQuoteService.mapObjectsToString(keyList);
    ...
    void toQuoteFromMapTest() {
        ...
        // under test
        Quote quote = cmcQuoteService.toQuoteFromMap(mapEntry);
